...
* The expanded private key returned by the standalone expansion routine is not scrubbed; ownership passes to the caller. The expansion inside signing and inside key generation uses a scrubbed working buffer.
...
The operational limit follows directly: **a restore reaches funds only if they were created under the digest-generator scheme.** A phrase from the browser extension restores the same accounts here and the same addresses. A wallet created by any of the other four clients does not, and this program gives no indication of the difference — it derives a valid, well-formed wallet whose accounts are empty, which is indistinguishable from a wallet that is genuinely empty.
...
**The ceiling was 20 until S15, and the two bounds no longer coincide.** BIP-44's 20 is a gap limit over unused addresses; the ceiling bounds how many spends an account has already made before a phrase alone cannot recover it, which is a quantity BIP-44 does not have, so the two shared a number and nothing else. 10,000 is taken from the only other implementation of this protocol — the shipped browser extension walks `0..10000` over the same key positions in `MasterSeed.deriveWotsIndexFromWotsAddrHash` — and those are the users whose funds a phrase restored here has to reach. The window stays 20: it bounds the size of a *disagreement*, which is the shape BIP-44's number does fit.
...
| 2 | StartupRefused | nothing ran: an unusable `--node`, no terminal, no entropy source, the store would not open, or the wallet refused to open because reconciliation found a divergence |
| 3 | Refused | the command was refused — every refusal from `create`, `address`, `discover`, `status`, `reconcile`, `restore` and `submit`, which open no wallet at all, and every refusal from a command that did open one, including a digest mismatch on `resign`, a `resign` whose reservation the chain has already moved past, and a socket write that failed |
...
- **There is no differential check against another implementation at run time.** The fixture corpus is what the crate is checked against; it was produced by executing the reference implementations and cannot be regenerated from this repository.
...
| release | **closed.** The crate is version `0.1.0` and is not published; the binary is built from source with the `mesh-https` transport feature enabled, and the minimum supported Rust version is 1.89. `v0.1.0` waited on item F — `restore` run against a live chain, which every other verb had — and item F was met on 2026-09-15: a mainnet run between blocks 1,086,967 and 1,086,987 in which `restore` reproduced a funded account from the phrase alone and `reconcile` caught a store up after a spend it did not make, with `discover`, `transaction`, `recent-transactions`, `block` and `blocks` running live beside them for the first time (AGENT.md, Known-open 33). The tag is placed at S17, on the commit that fixes the two defects that run found. `README.md` has been at the root since pull request #1, merged after S9, and `LICENSE.md` since S10b — the Mochimo Cryptocurrency Engine License Agreement, version 1.0, which the workspace declares as `license-file` and the crate inherits. This row said both were deliberately not added: true when S8 wrote it, false from S10b on, and corrected at S16 |
...
Group N is a live capture from a single deployment at a single block and is the one file that is not reproducible byte for byte: balances and block heights move. Its name is declared excluded from the regeneration control, though the declaration is only a statement — what actually excludes it is copying the committed file into the regenerated tree before the comparison. A re-run of the capture is a new capture whose diff is read, never a control that passes.
